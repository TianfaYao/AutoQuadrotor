...
//交叉加速度
  ...
  body_ul_SPE_z=body_ul_SPE_z*SPE_Z_ul_dFactor+(1-SPE_Z_ul_dFactor)*lastbody_ul_SPE_z;
	...
	body_ul_SPE_z_fc=body_ul_SPE_z_fc+imu.Eacc.z*dt;                    //加计直接积分
...
//加速度
	...
	accErr_ms=acccal_ms-imu.Eacc.z;                       //本来应该是0但是由于气压计的游走这个会发生该变
...
//	VAL_LIMIT(z_spe[ul].PID.expect,-MAX_Z_SPE,MAX_Z_SPE);
	...
	z_spe[ul].PID.ec=imu.Eacc.z*100;             //加速度的测量值直接作为阻尼
	//z_spe.Err_d(dt);
	z_spe[ul].Cal();
	//混合输出
...
if(Rc.g_CH[ksl]==dis_ms_z) 
{
  ...
  VAL_LIMIT(z_spe[ms].PID.expect,-MAX_Z_SPE,MAX_Z_SPE);
	...
	z_spe[ms].PID.ec=imu.Eacc.z*100;             //加速度的测量值直接作为阻尼
